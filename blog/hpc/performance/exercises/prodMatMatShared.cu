...
prodMul(const float *A, const float *B, float *C, int size)
{
    ...
    __shared__ float sharedB[MAXV];


    for (int idx = threadIdx.x; idx < size; idx+=(blockDim.x*size)){
        for (int j = 0; j < size; j++) {
           sharedB[idx+j] = B[idx+j];
        }
    }
    __syncthreads();
    ...
    if (i < size)
    {
        for (int j = 0; j < size; j++)
        {
          ...
          {
             sum += A[i * size + k] * sharedB[k*size + j];
          }
          C[i * size + j] = sum;
        }
    }
...
main(void)
{
    ...
    for (int i = 0; i < numElements*numElements; ++i)
    {
        ...
        h_B[i] = (float) 10*((i/numElements)+1);//rand()/(float)RAND_MAX;
    }
