...
main(void)
{
   ...
   // Verify that the result vector is correct
    float * goldC = (float *) malloc (size*size);
    for (int pRes = 0; pRes < numElements; ++pRes)
    {
 	for (int i = 0; i < numElements; i++)
        {
                goldC[pRes*numElements + i] =  h_A[pRes*numElements + i] + h_B[pRes*numElements + i];
        }
	
    }
    for (int i = 0; i < numElements; ++i)
    {
        //printf("%f %f,",goldC[i],h_C[i]);
        if (fabs(goldC[i] - h_C[i]) > 1e-2)
        {
